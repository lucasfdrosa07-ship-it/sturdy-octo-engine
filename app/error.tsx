...
export default function GlobalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { /* estado visual apenas */ }, [error]);
  return (
    <div className="min-h-dvh bg-bg flex flex-col items-center justify-center px-4 text-center">
      ...
      <div className="w-full max-w-[420px] rounded-xl border border-line bg-panel p-8 anim-up">
        ...
        <h1 className="text-[17px] font-semibold tracking-tight mt-3">Algo deu errado</h1>
        <p className="text-[13px] text-mut mt-2 leading-relaxed">
          Ocorreu um erro interno inesperado. Sua conversa e seus dados estão seguros — tente novamente.
        </p>
        {error.digest && <code className="inline-block mt-2 text-[10.5px] font-mono text-faint bg-soft border border-line rounded px-1.5 py-0.5">ref: {error.digest}</code>}
        <div className="mt-5 flex flex-col gap-2">
          <button onClick={() => reset()} className="inline-flex items-center justify-center gap-1.5 h-9 px-3.5 rounded-lg bg-ink text-bg text-[13.5px] font-medium hover:opacity-85 cursor-pointer">
            <Ic n={I.RefreshCw} s={15} />Tentar novamente
          </button>
